 ...
 * unchanged — the stream is a bonus channel, not a replacement.
 *
...
// Nest (CYD) MAC — update if your Nest MAC differs
static const uint8_t NEST_MAC[6] = {0xA4, 0xF0, 0x0F, 0x5D, 0x96, 0xD4};

// Scan summary packet — must match nest/nest.ino exactly
#define WASP_PKT_SUMMARY 0x01

typedef struct __attribute__((packed)) {
  uint8_t  type;          // WASP_PKT_SUMMARY
  uint8_t  workerMac[6];  // raw MAC bytes
  uint8_t  gpsFix;        // 1 = valid fix
  float    lat;
  float    lon;
  float    altM;
  uint8_t  sats;
  ...
  uint8_t  wifi5g;
  uint16_t bleCount;
  int8_t   bestRssi;      // strongest WiFi signal this cycle (0 if no networks)
  uint32_t cycleCount;
} scan_summary_t;
...
static void logWiFiRow(const String& mac, const String& ssid, wifi_auth_mode_t auth,
                       int channel, int rssi, double lat, double lon, double altM, double accuracy) {
...
static void onSendResult(const wifi_tx_info_t* txInfo, esp_now_send_status_t status) {
...
static void sendSummary(int wifiTotal, int wifi2g, int wifi5g,
  ...
  // Return radio to the ESP-NOW channel after WiFi scanning.
  esp_wifi_set_channel(ESPNOW_CHANNEL, WIFI_SECOND_CHAN_NONE);

  scan_summary_t pkt = {};
  pkt.type      = WASP_PKT_SUMMARY;
  pkt.gpsFix    = gps.location.isValid() ? 1 : 0;
  ...
  pkt.hdop      = gps.hdop.isValid()       ? gps.hdop.hdop() : 99.99f;
...
WiFiScanResult runWiFiScan() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (hasFix && sdOk && logFile)
      logWiFiRow(WiFi.BSSIDstr(i), ssid, WiFi.encryptionType(i), ch, rssi, lat, lon, alt, accuracy);
  }

  r.total = n;
  Serial.printf("[WORKER] WiFi: %d network(s) — %d x 2.4GHz, %d x 5GHz%s\n",
                n, r.g2, r.g5, hasFix ? "" : " | no GPS fix — not logged");
  WiFi.scanDelete();
  return r;
}
...
void setup() {
  ...
  Serial.println("\n========================================");
  Serial.println(" W.A.S.P. Worker — Stage 5");
  Serial.println(" GPS + SD Logging + ESP-NOW Streaming");
  Serial.println("========================================");
  ...
  // ESP-NOW — must init before NimBLE to avoid radio conflicts
  esp_wifi_set_channel(ESPNOW_CHANNEL, WIFI_SECOND_CHAN_NONE);
  if (esp_now_init() != ESP_OK) {
    Serial.println(" ESP-NOW init FAILED — streaming disabled");
  } else {
