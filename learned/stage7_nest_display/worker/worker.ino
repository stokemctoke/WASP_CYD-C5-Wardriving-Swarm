...
#define NEST_UPLOAD_URL "http://192.168.4.1/upload"
#define SYNC_EVERY      5    // sync after this many scan cycles
...
static void syncFiles() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    while (true) {
      ...
      File f = SD.open(path);
      if (!f) { Serial.println("open failed"); failed++; continue; }

      HTTPClient http;
      String url = String(NEST_UPLOAD_URL) + "?worker=" + myMac + "&file=" + name;
      http.begin(url);
...
void setup() {
  ...
  Serial.println("\n========================================");
  Serial.println(" W.A.S.P. Worker — Stage 6");
  Serial.println(" GPS + SD + ESP-NOW + File Sync");
  Serial.println("========================================");
  ...
  feedGPS(2000);
  Serial.printf(" Sync every %d cycles  AP: %s\n", SYNC_EVERY, NEST_AP_SSID);
...
void loop() {
  ...
  if (cycleCount % SYNC_EVERY == 0) syncFiles();
